...
def create_transformed_url(original_url, transformation_params, caption=None):
    ...
    # Add base transformations if they exist
    if transformation_params:
        all_params.append(transformation_params)

    # Add caption overlay if a caption is provided
    if caption:
        ...
        all_params.append(text_overlay)

    # Join all transformations with ':'
    # Filter out empty strings in case transformation_params was ""
    final_transformation_string = ":".join([p for p in all_params if p])

    if not final_transformation_string:
        return original_url

    parts = original_url.split("/")

    # Basic check for valid ImageKit URL structure
    if len(parts) < 5:
        return original_url
    ...
    return f"{base_url}/tr:{final_transformation_string}/{file_path}"
    ...
    if response.status_code == 200:
        ...
        for post in posts:
            ...
            if post['file_type'] == 'image':
                # Pass "" as base transform, but add caption
                uniform_url = create_transformed_url(post['url'], "", caption)
                st.image(uniform_url, width=300)
            else:
                # For videos: pass resize transform, but no caption overlay
                # (Caption is shown below with st.caption)
